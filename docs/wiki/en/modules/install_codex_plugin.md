...
**Imported by:** invoked as a script. Referenced from `Readme.md` and `CLAUDE.md` as the local-install command, and listed in `CHANGELOG.md` as the Codex installer. The `.codex-plugin/` manifest it packages is the Codex twin of `.claude-plugin/` — both `plugin.json` files are kept in lockstep at version `0.6.3`.
