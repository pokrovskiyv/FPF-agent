...
| Plugin manifest | `.claude-plugin/plugin.json` | Plugin name, `version` (`0.10.0`), keywords — consumed by Claude Code's plugin loader |
...
- **Dual-runtime packaging.** The project is packaged as a plugin for **both** Claude Code (`.claude-plugin/`) and Codex CLI (`.codex-plugin/` + `scripts/install_codex_plugin.py` + a home-local marketplace). It is not a Claude-Code-only artifact. Both manifests carry the same version (`0.10.0`); see [Versioning](#versioning) for how they stay in sync.
