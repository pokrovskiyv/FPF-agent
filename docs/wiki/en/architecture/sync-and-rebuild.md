...
The commit in Step 8 also triggers the PreToolUse hook, which appends a Conventional-Commit changelog entry and bumps `.claude-plugin/plugin.json` on its own. The current plugin version is **0.10.0**.
