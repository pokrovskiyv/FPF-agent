...
The project is **dual-packaged**: the same repository is a plugin for **Claude Code** (via `.claude-plugin/`) and for **Codex CLI** (via `.codex-plugin/` + a home-local marketplace). Current version: **0.6.0** (kept in lockstep across both manifests).
