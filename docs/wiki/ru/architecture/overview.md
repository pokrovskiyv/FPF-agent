...
Проект **упакован сразу для двух сред**: один и тот же репозиторий — это плагин и для **Claude Code** (через `.claude-plugin/`), и для **Codex CLI** (через `.codex-plugin/` плюс локальный marketplace в домашней директории). Текущая версия: **0.6.0** (держится синхронной в обоих манифестах).
