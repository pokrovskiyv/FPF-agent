...
Спецификация **First Principles Framework (FPF)** (~94 000 строк) плюс **навык**, применяющий паттерны FPF к координационным задачам пользователя, при этом сама терминология FPF никогда не попадает наружу.

Проект **упакован сразу для двух сред**: один и тот же репозиторий — это плагин и для **Claude Code** (через `.claude-plugin/`), и для **Codex CLI** (через `.codex-plugin/` плюс локальный marketplace в домашней директории). Текущая версия: **0.6.3** (держится синхронной в обоих манифестах).
...
| **Декомпозированные секции** | `sections/` | ~307 генерируемых файлов + metadata.json + маршруты + xrefs + FAISS-индекс |
