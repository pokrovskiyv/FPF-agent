...
- **Полная цепочка добавляет:** `C.2.LS` (U.LanguageStateFacetProfile — компактный профиль владельца), `A.16.2` (Reopen / SketchBackoff / Respecify), `B.4.1` (Observe → Notice → Stabilize → Route), `B.5.2.0` (U.AbductivePrompt).
...
| 2 | C.2.LS | U.LanguageStateFacetProfile — компактный профиль владельца | |
