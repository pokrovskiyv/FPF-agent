...
Активен. Используется для задачи `comparison_selection`. Шаблон резонёра: «Таблица критериев → Пробелы в данных → Рекомендация». Ключевое проектное ограничение: пустые ячейки остаются пустыми до сбора данных — никаких фальшивых ранжирований. Файл маршрута пересобирается скриптом `scripts/build_routes.py` из `sections/metadata.json`, поэтому заголовки секций и пути к файлам в цепочке синхронизируются со спецификацией при каждой пересборке.
