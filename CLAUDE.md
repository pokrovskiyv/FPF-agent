...
The **First Principles Framework (FPF)** specification (~105,000 lines) plus a **Claude Code Skill** that applies FPF to users' coordination problems — without exposing FPF terminology.

- `FPF-Spec.md` — upstream monolith (source of truth, do not edit directly)
- `sections/` — decomposed spec (~330 files), generated from the monolith
...
# Individual scripts
python3 scripts/split_spec.py          # FPF-Spec.md → sections/ (~330 files)
python3 scripts/build_metadata.py      # ToC → sections/metadata.json (320 entries)
...
**Do not read FPF-Spec.md directly** — it's ~105K lines. Instead:
