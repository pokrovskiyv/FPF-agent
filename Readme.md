...
[FPF](https://github.com/ailev/FPF) (First Principles Framework) — спецификация на 59 000 строк, которую создаёт и развивает [Анатолий Левенчук](https://ailev.livejournal.com), специалист по системной инженерии. По сути это **архитектура для мышления**: набор паттернов, которые структурируют координацию работы, сравнение альтернатив, разбор контрактов, проверку доверия к метрикам и десятки других задач. FPF даёт и людям, и AI-агентам воспроизводимый каркас — одни и те же категории, одна и та же логика разбора, независимо от модели и запуска.

Проблема в том, что спецификация огромная: 5.3 MB текста, ~1.3 миллиона токенов. Она не помещается ни в одно контекстное окно целиком. А даже если бы помещалась — модели сложно искать нужный паттерн среди 59 000 строк, особенно когда запрос на обычном языке.

**FPF-agent** решает эту проблему. Монолит разобран на 242 секции с поисковым индексом и маршрутами, а команда из 5 агентов подбирает нужные паттерны и применяет их к вашей задаче. Терминология FPF при этом остаётся за кулисами — вы формулируете задачу и получаете результат на обычном языке.
...
graph TD
    ...
    Meta --> MetaJSON["metadata.json<br/><b>242 записей</b><br/>ключевые слова, зависимости,<br/>пользовательские запросы"]
    ...
    Emb --> FAISS["FAISS индекс<br/><b>189 векторов × 1024 dim</b><br/><i>BAAI/bge-m3, мультиязычный</i>"]
