...
Проблема в том, что спецификация огромная: 8.3 MB текста, ~2.2 миллиона токенов. Она не помещается ни в одно контекстное окно целиком. А даже если бы помещалась — модели сложно искать нужный паттерн среди 86 000 строк, особенно когда запрос на обычном языке.
...
Полная спецификация — 8.3 MB, ~2.2M токенов. В контекстное окно на 1M она не помещается целиком.
...
|---|---|---|---|
| Контент спецификации | 3-8 секций (~6-50K) | 0 | Все 279 секций (~2.2M) |
| Агенты + метаданные + маршруты | ~45K | 0 | 0 |
| **Итого на инфраструктуру** | **55-104K** | **0** | **~2.2M (не помещается)** |
