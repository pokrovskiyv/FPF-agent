...
[FPF](https://github.com/ailev/FPF) (First Principles Framework) — спецификация на 97 000 строк, которую создаёт и развивает [Анатолий Левенчук](https://ailev.livejournal.com), специалист по системной инженерии. По сути это **архитектура для мышления**: набор паттернов, которые структурируют координацию работы, сравнение альтернатив, разбор контрактов, проверку доверия к метрикам и десятки других задач. FPF даёт и людям, и AI-агентам воспроизводимый каркас — одни и те же категории, одна и та же логика разбора, независимо от модели и запуска.

Проблема в том, что спецификация огромная: 9.7 MB текста, ~2.5 миллиона токенов. Она не помещается ни в одно контекстное окно целиком. А даже если бы помещалась — модели сложно искать нужный паттерн среди 97 000 строк, особенно когда запрос на обычном языке.

**FPF-agent** решает эту проблему. Монолит разобран на 310 секций с поисковым индексом и маршрутами, а команда из 5 агентов подбирает нужные паттерны и применяет их к вашей задаче. Терминология FPF при этом остаётся за кулисами — вы формулируете задачу и получаете результат на обычном языке.
...
Монолитная спецификация (97K строк) автоматически разбирается в поисковый индекс:

```mermaid
graph TD
    Spec["FPF-Spec.md<br/><b>97 255 строк</b>"] --> Split["split_spec.py"]
    Split --> Sections["310 секций<br/>в 15 директориях"]
    Sections --> Meta["build_metadata.py"]
    Meta --> MetaJSON["metadata.json<br/><b>306 записей</b><br/>ключевые слова, зависимости,<br/>пользовательские запросы"]
    ...
    Emb --> FAISS["FAISS индекс<br/><b>280 векторов × 1024 dim</b><br/><i>BAAI/bge-m3, мультиязычный</i>"]
...
На выходе 310 файлов в 15 директориях (~10 MB). Каждый файл можно загрузить отдельно, без остальных.
</details>
...
| Записей | 306 |
| Ключевых слов | 1 643 |
| Пользовательских запросов | 568 |
| Рёбер графа зависимостей | 2 783 |
| Записей с файловыми путями | 280 (92%) |
...
- 1024-dim вектор даёт достаточную выразительность для 280 секций
...
| Тип индекса | FAISS IndexFlatIP | Exact search — при 280 векторах compression не нужна |
...
| Показатель | Значение |
|-----------|---------|
| Проиндексировано секций | 280 (из 306 записей; 26 preface/ToC-запись без файла секции не индексируется) |
| Размер файла | 1120 KB = 280 × 1024 × 4 bytes (float32) |
...
Полная спецификация — 9.7 MB, ~2.5M токенов. В контекстное окно на 1M она не помещается целиком.

Пайплайн решает эту проблему: загружает 3-8 секций из 310 вместо всего монолита. Остальное — накладные расходы на агентов и метаданные.

| Что считаем | FPF-pipeline | Без FPF | Монолит (теоретически) |
|---|---|---|---|
| Контент спецификации | 3-8 секций (~6-50K) | 0 | Все 310 секций (~2.5M) |
| Агенты + метаданные + маршруты | ~45K | 0 | 0 |
| **Итого на инфраструктуру** | **55-104K** | **0** | **~2.5M (не помещается)** |
...
```
FPF-agent/
├── FPF-Spec.md              # Монолитная спецификация (97K строк, не читать напрямую)
├── sections/                 # Разложенная спецификация
│   ├── metadata.json         #   306 записей: паттерны, зависимости, ключевые слова
...
│   └── {15 директорий}/      #   310 файлов секций по частям A-K
...
# Отдельные шаги
python3 scripts/split_spec.py          # Спецификация → 310 секций
