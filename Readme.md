...
> Навык Claude Code, который применяет 59K-строчную спецификацию координации к вашим задачам — без терминологии, на любой модели.
...
FPF-agent не учит вас фреймворку и не работает как поисковик по спецификации. Он делает вашу обычную работу надёжнее.

Допустим, вы разбираете контракт — на выходе получите документ, где правила, условия, обязанности и доказательства разделены, а пробелы помечены. Сравниваете технологии — таблицу, где неизмеренные ячейки честно пусты, а не забиты предположениями. Проектируете оргструктуру — карту, где видно различие между "у нас есть описание процесса" и "мы реально это делаем".

За кулисами работает [FPF](https://github.com/ailev/FPF) (First Principles Framework) — спецификация на 59 000 строк с паттернами координации. Но в выводе вы этого не увидите: никакой терминологии, никаких идентификаторов паттернов.

Как устроен процесс:
1. Описываете задачу обычным языком
2. Система определяет, какие паттерны координации применимы
3. На выходе — структурированный результат в тех же терминах, что вы использовали

---

## Зачем это нужно

Любая сильная модель даст хороший совет. Проблема в другом: каждый раз — другой. Разберите один контракт — получите 5 категорий. Разберите следующий — 4 другие. Попросите коллегу — третий вариант.
...
На выходе — тот же ваш контракт, то же решение, та же оргструктура. Но с устойчивой структурой, которая не меняется от запуска к запуску.
...
Пайплайн экономит 95%+ контекстного окна по сравнению с загрузкой всей спецификации целиком.
