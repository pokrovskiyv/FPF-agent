...
[FPF](https://github.com/ailev/FPF) (First Principles Framework) — спецификация на 94 000 строк, которую создаёт и развивает [Анатолий Левенчук](https://ailev.livejournal.com), специалист по системной инженерии. По сути это **архитектура для мышления**: набор паттернов, которые структурируют координацию работы, сравнение альтернатив, разбор контрактов, проверку доверия к метрикам и десятки других задач. FPF даёт и людям, и AI-агентам воспроизводимый каркас — одни и те же категории, одна и та же логика разбора, независимо от модели и запуска.

Проблема в том, что спецификация огромная: 9.2 MB текста, ~2.4 миллиона токенов. Она не помещается ни в одно контекстное окно целиком. А даже если бы помещалась — модели сложно искать нужный паттерн среди 94 000 строк, особенно когда запрос на обычном языке.

**FPF-agent** решает эту проблему. Монолит разобран на 307 секций с поисковым индексом и маршрутами, а команда из 5 агентов подбирает нужные паттерны и применяет их к вашей задаче. Терминология FPF при этом остаётся за кулисами — вы формулируете задачу и получаете результат на обычном языке.
...
Монолитная спецификация (94K строк) автоматически разбирается в поисковый индекс:

```mermaid
graph TD
    Spec["FPF-Spec.md<br/><b>94 162 строк</b>"] --> Split["split_spec.py"]
    Split --> Sections["307 секций<br/>в 14 директориях"]
    Sections --> Meta["build_metadata.py"]
    Meta --> MetaJSON["metadata.json<br/><b>299 записей</b><br/>ключевые слова, зависимости,<br/>пользовательские запросы"]
    ...
    Emb --> FAISS["FAISS индекс<br/><b>277 векторов × 1024 dim</b><br/><i>BAAI/bge-m3, мультиязычный</i>"]
...
На выходе 307 файлов в 14 директориях (~9 MB). Каждый файл можно загрузить отдельно, без остальных.
</details>
...
| Записей | 299 |
| Ключевых слов | 1 647 |
| Пользовательских запросов | 571 |
| Рёбер графа зависимостей | 2 762 |
| Записей с файловыми путями | 277 (93%) |
...
- 1024-dim вектор даёт достаточную выразительность для 277 секций
...
| Тип индекса | FAISS IndexFlatIP | Exact search — при 277 векторах compression не нужна |
...
| Показатель | Значение |
|-----------|---------|
| Проиндексировано секций | 277 (из 299 записей; 22 preface/ToC-запись без файла секции не индексируется) |
| Размер файла | 1108 KB = 277 × 1024 × 4 bytes (float32) |
...
Полная спецификация — 9.2 MB, ~2.4M токенов. В контекстное окно на 1M она не помещается целиком.

Пайплайн решает эту проблему: загружает 3-8 секций из 307 вместо всего монолита. Остальное — накладные расходы на агентов и метаданные.

| Что считаем | FPF-pipeline | Без FPF | Монолит (теоретически) |
|---|---|---|---|
| Контент спецификации | 3-8 секций (~6-50K) | 0 | Все 307 секций (~2.4M) |
| Агенты + метаданные + маршруты | ~45K | 0 | 0 |
| **Итого на инфраструктуру** | **55-104K** | **0** | **~2.4M (не помещается)** |
...
```
FPF-agent/
├── FPF-Spec.md              # Монолитная спецификация (94K строк, не читать напрямую)
├── sections/                 # Разложенная спецификация
│   ├── metadata.json         #   299 записей: паттерны, зависимости, ключевые слова
...
│   └── {14 директорий}/      #   307 файлов секций по частям A-K
...
# Отдельные шаги
python3 scripts/split_spec.py          # Спецификация → 307 секций
