...
[FPF](https://github.com/ailev/FPF) (First Principles Framework) — спецификация на 107 000 строк, которую создаёт и развивает [Анатолий Левенчук](https://ailev.livejournal.com), специалист по системной инженерии. По сути это **архитектура для мышления**: набор паттернов, которые структурируют координацию работы, сравнение альтернатив, разбор контрактов, проверку доверия к метрикам и десятки других задач. FPF даёт и людям, и AI-агентам воспроизводимый каркас — одни и те же категории, одна и та же логика разбора, независимо от модели и запуска.

Проблема в том, что спецификация огромная: 13.2 MB текста, ~3.5 миллиона токенов. Она не помещается ни в одно контекстное окно целиком. А даже если бы помещалась — модели сложно искать нужный паттерн среди 107 000 строк, особенно когда запрос на обычном языке.

**FPF-agent** решает эту проблему. Монолит разобран на 343 секции с поисковым индексом и маршрутами, а команда из 5 агентов подбирает нужные паттерны и применяет их к вашей задаче. Терминология FPF при этом остаётся за кулисами — вы формулируете задачу и получаете результат на обычном языке.
...
Монолитная спецификация (107K строк) автоматически разбирается в поисковый индекс:

```mermaid
graph TD
    Spec["FPF-Spec.md<br/><b>107 170 строк</b>"] --> Split["split_spec.py"]
    Split --> Sections["343 секции<br/>в 15 директориях"]
    Sections --> Meta["build_metadata.py"]
    Meta --> MetaJSON["metadata.json<br/><b>348 записей</b><br/>ключевые слова, зависимости,<br/>пользовательские запросы"]
    ...
    Emb --> FAISS["FAISS индекс<br/><b>312 векторов × 1024 dim</b><br/><i>BAAI/bge-m3, мультиязычный</i>"]
...
На выходе 343 файла в 15 директориях (~13 MB). Каждый файл можно загрузить отдельно, без остальных.
</details>
...
| Записей | 348 |
| Ключевых слов | 1 628 |
| Пользовательских запросов | 685 |
| Рёбер графа зависимостей | 3 759 |
| Записей с файловыми путями | 312 (90%) |
...
- 1024-dim вектор даёт достаточную выразительность для 312 секций
...
| Тип индекса | FAISS IndexFlatIP | Exact search — при 312 векторах compression не нужна |
...
| Показатель | Значение |
|-----------|---------|
| Проиндексировано секций | 312 (из 348 записей; 36 preface/ToC-запись без файла секции не индексируется) |
| Размер файла | 1248 KB = 312 × 1024 × 4 bytes (float32) |
...
Полная спецификация — 13.2 MB, ~3.5M токенов. В контекстное окно на 1M она не помещается целиком.

Пайплайн решает эту проблему: загружает 3-8 секций из 343 вместо всего монолита. Остальное — накладные расходы на агентов и метаданные.

| Что считаем | FPF-pipeline | Без FPF | Монолит (теоретически) |
|---|---|---|---|
| Контент спецификации | 3-8 секций (~6-50K) | 0 | Все 343 секции (~3.5M) |
| Агенты + метаданные + маршруты | ~45K | 0 | 0 |
| **Итого на инфраструктуру** | **55-104K** | **0** | **~3.5M (не помещается)** |
...
```
FPF-agent/
├── FPF-Spec.md              # Монолитная спецификация (107K строк, не читать напрямую)
├── sections/                 # Разложенная спецификация
│   ├── metadata.json         #   348 записей: паттерны, зависимости, ключевые слова
...
│   └── {15 директорий}/      #   343 файла секций по частям A-K
...
# Отдельные шаги
python3 scripts/split_spec.py          # Спецификация → 343 секции
