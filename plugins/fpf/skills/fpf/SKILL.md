...
current project directory. For the home-local install created by this repo's
installer, the plugin root is `~/plugins/fpf`. For source checkout development,
the plugin root is `plugins/fpf` inside the FPF-agent repo. If the Codex
environment shows a more specific plugin source path, use that directory.

When reading referenced files, always prefix paths with the resolved plugin
root. For example, `agents/fpf-classifier.md` means
...
The upstream sync prompt is dev-side infrastructure for this repository and is
not packaged or used during skill execution.
